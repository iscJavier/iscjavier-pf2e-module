...
const SourceEntryPoint = './src/main.ts';
const Directories = {
  Dist: './dist/',
  Styles: './styles/',
  Lang: './lang/',
...
const outputLanguagesHandler = (outDir) =>
  outputFilesHandler(`${Directories.Lang}${MATCH_PUBLIC}`, `${outDir}${Directories.Lang}`);
const outputTemplatesHandler = (outDir) =>
  outputFilesHandler(`${Directories.Templates}${MATCH_PUBLIC}`, `${outDir}${Directories.Templates}`);
const outputMetaFilesHandler = (outDir) => outputFilesHandler(['./LICENSE', './README.md'], outDir);

const build = (outDir) =>
  gulp.series(
    deleteDistDirHandler(outDir),
    gulp.parallel(buildStylesHandler(outDir), buildSourceHandler(outDir), buildManifestHandler(outDir)),
    gulp.parallel(outputLanguagesHandler(outDir), outputTemplatesHandler(outDir), outputMetaFilesHandler(outDir)),
  );

const compressDist = () => {
  const subDir = `${Directories.Dist}/${Package.name}/`;
  const copyDistToSubdir = () => gulp.src(`${Directories.Dist}${MATCH_PUBLIC}`).pipe(gulp.dest(subDir));
  const zipSubdir = () =>
    gulp
      .src(`${subDir}${MATCH_PUBLIC}`)
      .pipe(zip(`${Package.name}.zip`))
      .pipe(gulp.dest(Directories.Bundle));
  const copyModuleJSON = () => gulp.src(`${Directories.Dist}module.json`).pipe(gulp.dest(Directories.Bundle));
  return gulp.series(
